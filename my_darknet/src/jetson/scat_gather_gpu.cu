...
size_t scatter_gpu(scalar_t* src, struct iovec* iov, size_t count)
{
  size_t i, size=0;
  scalar_t* ptr = src;
  for (i = 0 ; i < count ; ++i) {
    checkCUDA(cudaMemcpy(iov[i].iov_base, ptr, sizeof(scalar_t)*iov[i].iov_len, cudaMemcpyDeviceToHost));
...
size_t gather_gpu(scalar_t* dst, struct iovec* iov, size_t count)
{
  size_t i, size=0;
  scalar_t* ptr = dst;
  for (i = 0 ; i < count ; ++i) {
    checkCUDA(cudaMemcpy(ptr, iov[i].iov_base, sizeof(scalar_t)*iov[i].iov_len, cudaMemcpyHostToDevice));
    size += sizeof(scalar_t)*iov[i].iov_len;
    ptr += iov[i].iov_len;
  }
  return size;
...
extern "C" size_t tile_cpy4d_gpu( scalar_t* dst, scalar_t* src, int start_idx,
                                  ...
                                  int flag)
{
  ...
  if (flag == HOST_TO_DEVICE) { // Gather.
    size = gather_gpu(dst, iov, count);
  } else
  if (flag == DEVICE_TO_HOST) { // Scatter.
    size = scatter_gpu(src, iov, count);
  } else {
    fprintf(stderr, "Invalid flag(<2): %d\n", flag);
    exit(0);
  }
