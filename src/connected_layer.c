...
layer make_connected_layer(int batch, int inputs, int outputs, ACTIVATION activation, int batch_normalize, int adam)
{
    ...
    fprintf(cfg_f, "\n[fully_connected]\n");
    fprintf(cfg_f, "input(%d)\n", inputs);
    fprintf(cfg_f, "output(%d)\n", outputs);

    if (batch_normalize) {
      fprintf(cfg_f, "\n[batch_normalize]\n");
      fprintf(cfg_f, "(%d,1,1)  # c*h*w\n", outputs);
    } else {
      fprintf(cfg_f, "\n[bias]\n");
      fprintf(cfg_f, "(1,%d,1,1)  # n*c*h*w\n", outputs);
    }

    fprintf(cfg_f, "\n[activation]\n");
    fprintf(cfg_f, "%s\n", get_activation_string(l.activation));
    fprintf(cfg_f, "(%d,%d,1,1)  # n*c*h*w\n", l.batch, outputs);
...
void forward_connected_layer(layer l, network net)
{
    ...
    float *c = l.output;
#ifdef EXTRACT
    printf("Dump connected weight. (%d)\n", n*k);
    FILE *dump = fopen("weight.dump", "ab");
    fwrite((char*)b, sizeof(float), n*k, dump);
    fclose(dump);
#endif
...
#ifdef EXTRACT
      printf("Dump connected bias. (%d)\n", n);
      FILE *dump = fopen("weight.dump", "ab");
      fwrite((char*)l.biases, sizeof(float), n, dump);
      fclose(dump);
#endif
